# [FUNCTIONS] Home Team Last 1st 2nd 3rd Weeks' Shots in Home Games
def func_HOME_LG_SHOTS_OT_H_L1W(row):
...
def func_HOME_LG_SHOTS_OT_H_L2W(row):
    filtered_df = main[main['HomeTeam']==row['HomeTeam']]
    filtered_df = filtered_df[filtered_df['Date']<row['Date']]
    try:
...
def func_HOME_LG_SHOTS_OT_H_L3W(row):
    filtered_df = main[main['HomeTeam']==row['HomeTeam']]
    filtered_df = filtered_df[filtered_df['Date']<row['Date']]
    try:
