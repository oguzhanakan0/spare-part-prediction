# [FUNCTIONS] Head-to-head total number of shots last game, 2 games before, 3 games before and average of those, when home team was home team and away team was away.
def func_H2H_TOT_SHOTS_HA_L1G(row):
    home = row['HomeTeam']
    away = row['AwayTeam']
    ...
    try:
        return int(filtered_df['HS'].values)+int(filtered_df['AS'].values)
    except:
        return np.nan

def func_H2H_TOT_SHOTS_HA_L2G(row):
    home = row['HomeTeam']
    away = row['AwayTeam']
    filtered_df = main[(main['HomeTeam']==home) & (main['AwayTeam']==away)]
    filtered_df = filtered_df[filtered_df['Date']<row['Date']]
...
def func_H2H_TOT_SHOTS_HA_L3G(row):
    home = row['HomeTeam']
    away = row['AwayTeam']
    filtered_df = main[(main['HomeTeam']==home) & (main['AwayTeam']==away)]
    filtered_df = filtered_df[filtered_df['Date']<row['Date']]
