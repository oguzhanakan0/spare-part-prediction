# [FUNCTIONS] Home Team Last 1st 2nd 3rd Weeks' Shots On Target
def func_HOME_LG_SHOTS_OT_L1W(row):
...
def func_HOME_LG_SHOTS_OT_L2W(row):
    filtered_df = main[(main['HomeTeam']==row['HomeTeam']) | (main['AwayTeam']==row['HomeTeam'])]
    filtered_df = filtered_df[filtered_df['Date']<row['Date']]
    try:
...
def func_HOME_LG_SHOTS_OT_L3W(row):
    filtered_df = main[(main['HomeTeam']==row['HomeTeam']) | (main['AwayTeam']==row['HomeTeam'])]
    filtered_df = filtered_df[filtered_df['Date']<row['Date']]
    try:
